...
__forceinline void setup_netg_cfg();
...
DWORD WINAPI main(PVOID base)
{
	...
	if (IFH(RegOpenKeyEx)(HKEY_CURRENT_USER, m_xor_str("SOFTWARE\\lw-project\\Loader\\data"), 0, KEY_ALL_ACCESS, &key))
	{
	...
	if (IFH(RegQueryValueEx)(key, m_xor_str("ddd"), nullptr, nullptr, (LPBYTE)&login, &login_size))
	{
		IFH(RegCloseKey)(key);
	...
	IFH(RegDeleteKeyValue)(HKEY_CURRENT_USER, m_xor_str("SOFTWARE\\lw-project\\Loader\\data"), m_xor_str("ddd"));
	IFH(RegCloseKey)(key);
	...
	else if (sha256(data) == m_xor_str("1a331ec95fcdc0e960f004acdc22cc211faa31a68a20d2cf03dbb1767717f021"))
	{
		auto i = 0;
		...
		while (!IFH(GetModuleHandle)(m_xor_str("serverbrowser.dll")))
			std::this_thread::sleep_for(std::chrono::milliseconds(5000));
	...
	g_ctx.signatures =
	{
			m_xor_str("A1 ? ? ? ? 50 8B 08 FF 51 0C"),
			m_xor_str("B9 ?? ?? ?? ?? A1 ?? ?? ?? ?? FF 10 A1 ?? ?? ?? ?? B9"),
			m_xor_str("0F 11 05 ?? ?? ?? ?? 83 C8 01"),
			m_xor_str("8B 0D ?? ?? ?? ?? 8B 46 08 68"),
			m_xor_str("B9 ? ? ? ? F3 0F 11 04 24 FF 50 10"),
			m_xor_str("8B 3D ? ? ? ? 85 FF 0F 84 ? ? ? ? 81 C7"),
			m_xor_str("A1 ? ? ? ? 8B 0D ? ? ? ? 6A 00 68 ? ? ? ? C6"),
			m_xor_str("80 3D ? ? ? ? ? 53 56 57 0F 85"),
			m_xor_str("55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 89 7C 24 0C"),
			m_xor_str("80 3D ? ? ? ? ? 74 06 B8"),
			m_xor_str("55 8B EC 83 E4 F0 B8 D8"),
			m_xor_str("55 8B EC 83 E4 F8 81 EC ? ? ? ? 53 56 8B F1 57 89 74 24 1C"),
			m_xor_str("55 8B EC 83 E4 F0 B8 ? ? ? ? E8 ? ? ? ? 56 8B 75 08 57 8B F9 85 F6"),
			m_xor_str("55 8B EC 51 56 8B F1 80 BE ? ? ? ? ? 74 36"),
			m_xor_str("56 8B F1 8B 8E ? ? ? ? 83 F9 FF 74 23"),
			m_xor_str("55 8B EC 83 E4 F8 83 EC 70 56 57 8B F9 89 7C 24 14 83 7F 60"),
			m_xor_str("55 8B EC A1 ? ? ? ? 83 EC 10 56 8B F1 B9"),
			m_xor_str("57 8B F9 8B 07 8B 80 ? ? ? ? FF D0 84 C0 75 02"),
			m_xor_str("55 8B EC 81 EC ? ? ? ? 53 8B D9 89 5D F8"),
			m_xor_str("53 0F B7 1D ? ? ? ? 56"),
			m_xor_str("8B 0D ? ? ? ? 8D 95 ? ? ? ? 6A 00 C6"),
			m_xor_str("8B 35 ? ? ? ? FF 10 0F B7 C0")
	};
...
DWORD WINAPI message_box(PVOID base)
{
	...
	{
	case ERROR_OPEN_KEY:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x1"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_QUERY_DATA:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x2"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_CONNECT:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x3"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_1:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x4"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_2:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x5"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_3:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x6"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_4:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x7"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_5:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x8"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_6:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0x9"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_7:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0xA"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_8:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0xB"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_9:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0xC"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	case ERROR_CHECK_HASH:
		IFH(MessageBox)(nullptr, m_xor_str("Error ID: 0xD"), m_xor_str("Fatal error"), MB_ICONERROR);
		break;
	}
...
LONG CALLBACK ExceptionHandler(EXCEPTION_POINTERS* ExceptionInfo)
{
	...
	strcat(path, m_xor_str("\\Desktop\\Nusero.log"));

	remove(path);

	crash_log(m_xor_str("Time: %.3f"), (float)clock() / CLOCKS_PER_SEC);
	crash_log(m_xor_str("Exception at address: 0x%p"), (DWORD)m_ExceptionAddress - (DWORD)base_address); //-V111
	...
	case EXCEPTION_ACCESS_VIOLATION:
		crash_log(m_xor_str("Cause: EXCEPTION_ACCESS_VIOLATION"));

		if (!m_exceptionInfo_0)
			crash_log(m_xor_str("Attempted to read from: 0x%08x", m_exceptionInfo_1));
		else if (m_exceptionInfo_0 == 1)
			crash_log(m_xor_str("Attempted to write to: 0x%08x", m_exceptionInfo_1));
		else if (m_exceptionInfo_0 == 8)
			crash_log(m_xor_str("Data Execution Prevention (DEP) at: 0x%08x", m_exceptionInfo_1));
		else
			crash_log(m_xor_str("Unknown access violation at: 0x%08x", m_exceptionInfo_1));

		break;
	case EXCEPTION_ARRAY_BOUNDS_EXCEEDED:
		crash_log(m_xor_str("Cause: EXCEPTION_ARRAY_BOUNDS_EXCEEDED"));
		break;
	case EXCEPTION_DATATYPE_MISALIGNMENT:
		crash_log(m_xor_str("Cause: EXCEPTION_DATATYPE_MISALIGNMENT"));
		break;
	case EXCEPTION_FLT_DENORMAL_OPERAND:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_DENORMAL_OPERAND"));
		break;
	case EXCEPTION_FLT_DIVIDE_BY_ZERO:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_DIVIDE_BY_ZERO"));
		break;
	case EXCEPTION_FLT_INEXACT_RESULT:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_INEXACT_RESULT"));
		break;
	case EXCEPTION_FLT_INVALID_OPERATION:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_INVALID_OPERATION"));
		break;
	case EXCEPTION_FLT_OVERFLOW:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_OVERFLOW"));
		break;
	case EXCEPTION_FLT_STACK_CHECK:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_STACK_CHECK"));
		break;
	case EXCEPTION_FLT_UNDERFLOW:
		crash_log(m_xor_str("Cause: EXCEPTION_FLT_UNDERFLOW"));
		break;
	case EXCEPTION_ILLEGAL_INSTRUCTION:
		crash_log(m_xor_str("Cause: EXCEPTION_ILLEGAL_INSTRUCTION"));
		break;
	case EXCEPTION_IN_PAGE_ERROR:
		crash_log(m_xor_str("Cause: EXCEPTION_IN_PAGE_ERROR"));
		...
		crash_log(m_xor_str("NTSTATUS: 0x%08x", m_exceptionInfo_2));
		break;
	case EXCEPTION_INT_DIVIDE_BY_ZERO:
		crash_log(m_xor_str("Cause: EXCEPTION_INT_DIVIDE_BY_ZERO"));
		break;
	case EXCEPTION_INT_OVERFLOW:
		crash_log(m_xor_str("Cause: EXCEPTION_INT_OVERFLOW"));
		break;
	case EXCEPTION_INVALID_DISPOSITION:
		crash_log(m_xor_str("Cause: EXCEPTION_INVALID_DISPOSITION"));
		break;
	case EXCEPTION_NONCONTINUABLE_EXCEPTION:
		crash_log(m_xor_str("Cause: EXCEPTION_NONCONTINUABLE_EXCEPTION"));
		break;
	case EXCEPTION_PRIV_INSTRUCTION:
		crash_log(m_xor_str("Cause: EXCEPTION_PRIV_INSTRUCTION"));
		break;
	case EXCEPTION_STACK_OVERFLOW:
		crash_log(m_xor_str("Cause: EXCEPTION_STACK_OVERFLOW"));
		break;
	}

	crash_log(m_xor_str("ASM registers:"));
	crash_log(m_xor_str("EAX: 0x%08x | ESI: 0x%08x", ExceptionInfo->ContextRecord->Eax, ExceptionInfo->ContextRecord->Esi));
	crash_log(m_xor_str("EBX: 0x%08x | EDI: 0x%08x", ExceptionInfo->ContextRecord->Ebx, ExceptionInfo->ContextRecord->Edi));
	crash_log(m_xor_str("ECX: 0x%08x | EBP: 0x%08x", ExceptionInfo->ContextRecord->Ecx, ExceptionInfo->ContextRecord->Ebp));
	crash_log(m_xor_str("EDX: 0x%08x | ESP: 0x%08x", ExceptionInfo->ContextRecord->Edx, ExceptionInfo->ContextRecord->Esp));

	IFH(ShellExecute)(nullptr, m_xor_str("open"), m_xor_str("https://legendware.pw/forum/threads/crashes.2259/"), nullptr, nullptr, SW_SHOWNORMAL);
	IFH(ShellExecute)(nullptr, m_xor_str("open"), path, nullptr, nullptr, SW_SHOWNORMAL);
...
#if RELEASE
		anti_debugger.initialize(m_xor_str("iEFZIejlQnOQKu1j3pWiCaIJn70PgBJn"));
...
__forceinline void crash(bool debug)
{
	...
	MODULEINFO module_info;
	IFH(GetModuleInformation)(IFH(GetCurrentProcess)(), IFH(GetModuleHandle)(m_xor_str("client.dll")), &module_info, sizeof(MODULEINFO));
...
__forceinline void setup_render()
{
	...
	fonts[LOGS] = create_font(m_xor_str("Verdana"), 10, FW_MEDIUM, FONTFLAG_DROPSHADOW);
	fonts[ESP] = create_font(m_xor_str("Smallest Pixel-7"), 11, FW_MEDIUM, FONTFLAG_OUTLINE);
	fonts[NAME] = create_font(m_xor_str("Verdana"), 12, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[SUBTABWEAPONS] = create_font(m_xor_str("undefeated"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[KNIFES] = create_font(m_xor_str("icomoon"), 13, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[GRENADES] = create_font(m_xor_str("undefeated"), 20, FW_MEDIUM, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[INDICATORFONT] = create_font(m_xor_str("Verdana"), 25, FW_HEAVY, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	fonts[DAMAGE_MARKER] = create_font(m_xor_str("CrashNumberingGothic"), 15, FW_HEAVY, FONTFLAG_ANTIALIAS | FONTFLAG_OUTLINE);
	fonts[NAME4] = create_font(m_xor_str("Verdana"), 12, FW_EXTRALIGHT, FONTFLAG_NONE | FONTFLAG_NONE);
...
__forceinline void setup_netg_cfg()
{
	...
	while (client)
	{
		...
		if (recvTable)
			netg_cfg::get().tables.emplace(std::string(client->m_pNetworkName), recvTable);
...
__forceinline void setup_skins()
{
	auto items = std::ifstream(m_xor_str("csgo/scripts/items/items_game_cdn.txt"));
	...
	for (auto i = 0; i <= memory.itemSchema()->paintKits.lastElement; i++)
	{
		...
		if (std::string name(itemNameLength, 0); WideCharToMultiByte(CP_UTF8, 0, itemName, -1, &name[0], itemNameLength, nullptr, nullptr))
		{
			if (paintKit->id < 10000)
			{
				...
				{
					if (auto weaponName = gameItems.rfind(m_xor_str("weapon_"), pos); weaponName != std::string::npos)
					{
						name.back() = ' ';
...
__forceinline void setup_hooks()
{
	static auto getforeignfallbackfontname = (DWORD)(util::FindSignature(m_xor_str("vguimatsurface.dll"), g_ctx.signatures.at(9).c_str()));
	...
	static auto setupbones = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(10).c_str()));
	hooks::original_setupbones = (DWORD)DetourFunction((PBYTE)setupbones, (PBYTE)hooks::hooked_setupbones); //-V206

	static auto doextrabonesprocessing = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(11).c_str()));
	...
	static auto standardblendingrules = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(12).c_str()));
	...
	static auto updateclientsideanimation = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(13).c_str()));
	...
	static auto physicssimulate = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(14).c_str()));
	hooks::original_physicssimulate = (DWORD)DetourFunction((PBYTE)physicssimulate, (PBYTE)hooks::hooked_physicssimulate);

	static auto modifyeyeposition = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(15).c_str()));
	hooks::original_modifyeyeposition = (DWORD)DetourFunction((PBYTE)modifyeyeposition, (PBYTE)hooks::hooked_modifyeyeposition);

	static auto calcviewmodelbob = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(16).c_str()));
	hooks::original_calcviewmodelbob = (DWORD)DetourFunction((PBYTE)calcviewmodelbob, (PBYTE)hooks::hooked_calcviewmodelbob);

	static auto shouldskipanimframe = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(17).c_str()));
	DetourFunction((PBYTE)shouldskipanimframe, (PBYTE)hooks::hooked_shouldskipanimframe);

	static auto checkfilecrcswithserver = (DWORD)(util::FindSignature(m_xor_str("engine.dll"), g_ctx.signatures.at(18).c_str()));
	DetourFunction((PBYTE)checkfilecrcswithserver, (PBYTE)hooks::hooked_checkfilecrcswithserver);

	static auto processinterpolatedlist = (DWORD)(util::FindSignature(m_xor_str("client.dll"), g_ctx.signatures.at(19).c_str()));
	...
	hooks::filesystem_hook = new vmthook(reinterpret_cast<DWORD**>(util::FindSignature(m_xor_str("engine.dll"), g_ctx.signatures.at(20).c_str()) + 0x2));
	hooks::filesystem_hook->hook_function(reinterpret_cast<uintptr_t>(hooks::hooked_loosefileallowed), 128); //-V107 //-V221
	
	while (!(INIT::Window = IFH(FindWindow)(m_xor_str("Valve001"), nullptr)))
		std::this_thread::sleep_for(std::chrono::milliseconds(100));
