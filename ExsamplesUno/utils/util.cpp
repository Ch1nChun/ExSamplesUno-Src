...
namespace util
{
	...
	bool is_button_down(int code)
	{
		...
		static auto cl_mouseenable = m_cvar()->FindVar(m_xor_str("cl_mouseenable"));
	...
	void movement_fix(Vector& wish_angle, CUserCmd* m_pcmd)
	{
		...
		static auto cl_upspeed = m_cvar()->FindVar(m_xor_str("cl_upspeed"));
	...
	{
		auto found = false;
		auto var = material->FindVar(m_xor_str("$envmaptint"), &found);
	...
	void create_state(c_baseplayeranimationstate* state, player_t* e)
	{
		...
		static auto fn = reinterpret_cast <Fn> (util::FindSignature(m_xor_str("client.dll"), m_xor_str("55 8B EC 56 8B F1 B9 ? ? ? ? C7 46")));
	...
	void update_state(c_baseplayeranimationstate* state, const Vector& angles)
	{
		...
		static auto fn = reinterpret_cast <Fn> (util::FindSignature(m_xor_str("client.dll"), m_xor_str("55 8B EC 83 E4 F8 83 EC 18 56 57 8B F9 F3 0F 11 54 24")));
	...
	void reset_state(c_baseplayeranimationstate* state)
	{
		...
		static auto fn = reinterpret_cast <Fn> (util::FindSignature(m_xor_str("client.dll"), m_xor_str("56 6A 01 68 ? ? ? ? 8B F1")));
	...
	{
		static auto cl_interp = m_cvar()->FindVar(m_xor_str("cl_interp")); //-V807
		static auto cl_interp_ratio = m_cvar()->FindVar(m_xor_str("cl_interp_ratio"));
		static auto sv_client_min_interp_ratio = m_cvar()->FindVar(m_xor_str("sv_client_min_interp_ratio"));
		static auto sv_client_max_interp_ratio = m_cvar()->FindVar(m_xor_str("sv_client_max_interp_ratio"));
		static auto cl_updaterate = m_cvar()->FindVar(m_xor_str("cl_updaterate"));
		static auto sv_minupdaterate = m_cvar()->FindVar(m_xor_str("sv_minupdaterate"));
		static auto sv_maxupdaterate = m_cvar()->FindVar(m_xor_str("sv_maxupdaterate"));
