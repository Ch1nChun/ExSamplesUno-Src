...
struct shot_info
{
	...
	std::string target_name = m_xor_str("None");
	std::string result = m_xor_str("None");

	std::string client_hitbox = m_xor_str("None");
	std::string server_hitbox = m_xor_str("None");
...
public:
	struct Globals  //-V730
	{
		...
		std::string time = m_xor_str("unknown");
