...
void Netg_cfg::restore() noexcept
{
	for (auto clientClass = m_client()->GetAllClasses(); clientClass; clientClass = clientClass->m_pNext)
...
void Netg_cfg::walkTable(bool unload, const char* networkName, RecvTable* recvTable, const std::size_t offset) noexcept
{
	for (auto i = 0; i < recvTable->propCount; i++)
	{
		...
		if (fnv::hashRuntime(prop.name) == fnv::hash(m_xor_str("baseclass")))
			continue;
		...
		const auto hash{ fnv::hashRuntime((networkName + std::string { m_xor_str("->") } + prop.name).c_str()) };

		constexpr auto getHook
		{ 
			[](uint32_t hash) noexcept -> recvProxy 
			{
				 if (hash == fnv::hash(m_xor_str("CBaseViewModel->m_nSequence")))
					 return viewModelSequence;

				 if (hash == fnv::hash(m_xor_str("CBaseEntity->m_bSpotted")))
					 return spottedHook;

				 if (hash == fnv::hash(m_xor_str("CBaseEntity->m_flSimulationTime")))
