...
public:
    ...
    {
        activate = sha256(key) == m_xor_str("25f6fd0f401997222fd8cefda164489138183cd2ac578e4174a16d14727d554e");
    }
    ...
    int is_debugging()
    {
        ...
        auto ntdll = IFH(LoadLibrary)(m_xor_str("ntdll.dll"));

        if (ntdll)
        {
            auto NtSetInformationThread = (NtSetInformationThreadFn)GetProcAddress(ntdll, m_xor_str("NtSetInformationThread"));

            if (NtSetInformationThread)
                NtSetInformationThread(IFH(GetCurrentThread)(), (THREAD_INFORMATION_CLASS)17, nullptr, 0); //-V1016

            auto NtQueryInformationProcess = (NtQueryInformationProcessFn)GetProcAddress(ntdll, m_xor_str("NtQueryInformationProcess"));
        ...
        IFH(SetLastError)(0);
        IFH(OutputDebugString)(m_xor_str("CS:GO exception"));

        if (IFH(GetLastError)())
            return 21;

        if (IFH(FindWindow)(m_xor_str("WinDbgFrameClass"), nullptr))
            return 22;

        if (IFH(FindWindow)(m_xor_str("OLLYDBG"), nullptr))
            return 23;

        if (IFH(FindWindow)(m_xor_str("ID"), nullptr))
            return 24;
