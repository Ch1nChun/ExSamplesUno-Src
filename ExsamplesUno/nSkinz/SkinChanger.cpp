...
static attributableitem_t* make_glove(int entry, int serial) noexcept
{
	...
	static auto Fn = util::find_pattern(m_xor_str("client.dll"), m_xor_str("\x55\x8B\xEC\x83\xE4\xF8\x51\x53\x56\x57\x8B\xF1"), m_xor_str("xxxxxxxxxxxx"));
...
{
    if (auto hud_weapons = memory.findHudElement(memory.hud, m_xor_str("CCSGO_HudWeaponSelection")) - 0x28)
		...
		if (player_model)
		{
			if (!g_ctx.globals.backup_model)
			{
				...
				if (model)
				{
					...
					if (studio_model)
					{
						auto name = m_xor_str("models/") + (std::string)studio_model->szName;
...
{
	if (auto iconOverride = iconOverrides[event->GetString(m_xor_str("weapon"))])
		event->SetString(m_xor_str("weapon"), iconOverride);
}