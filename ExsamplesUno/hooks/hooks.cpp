...
#include "../fonts/EvolutionIconsQunion.h"
...
namespace hooks
{
	...
	void GUI_Init(IDirect3DDevice9* pDevice)
	{
		...
		c_menu::get().futura_largers = io.Fonts->AddFontFromMemoryTTF((void*)futuramediumc, sizeof(futuramediumc), 32.f, &m_config, ranges);
		c_menu::get().icon_font = io.Fonts->AddFontFromMemoryTTF((void*)Icons, sizeof(Icons), 20, &m_config, io.Fonts->GetGlyphRangesDefault());
		c_menu::get().font_desc = io.Fonts->AddFontFromFileTTF("../../misc/fonts/Museo Sans Cyrl 700.ttf", 25.0f, NULL, io.Fonts->GetGlyphRangesJapanese());
		...
		c_menu::get().g_pNameFont = io.Fonts->AddFontFromMemoryTTF(MontserratExtraBold, sizeof(MontserratExtraBold), 30, NULL, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().g_pBigIconsFont = io.Fonts->AddFontFromMemoryTTF(EvolutionIconsQunion, sizeof(EvolutionIconsQunion), 51, &m_config, io.Fonts->GetGlyphRangesDefault());
		c_menu::get().g_pIconsWatermarkFont = io.Fonts->AddFontFromMemoryTTF(EvolutionIconsQunion, sizeof(EvolutionIconsQunion), 29, &m_config, io.Fonts->GetGlyphRangesDefault());
		c_menu::get().g_pIconsFont = io.Fonts->AddFontFromMemoryTTF(EvolutionIconsQunion, sizeof(EvolutionIconsQunion), 25, &m_config, io.Fonts->GetGlyphRangesDefault());
		c_menu::get().g_pTabsFont = io.Fonts->AddFontFromMemoryTTF(MontserratMedium, sizeof(MontserratMedium), 15, NULL, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().g_pTabsFont1 = io.Fonts->AddFontFromMemoryTTF(MontserratMedium, sizeof(MontserratMedium), 30, NULL, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().g_pMenuFont = io.Fonts->AddFontFromMemoryTTF(MontserratRegular, sizeof(MontserratRegular), 18, NULL, io.Fonts->GetGlyphRangesCyrillic());
		c_menu::get().g_pLoaderFont = io.Fonts->AddFontFromMemoryTTF(MontserratRegular, sizeof(MontserratRegular), 15, NULL, io.Fonts->GetGlyphRangesCyrillic());
		...
		if (c_menu::get().logo == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(
				...
				(PALETTEENTRY*)NULL, &c_menu::get().logo);
		}

		if (c_menu::get().loggo == nullptr)
		{

		}

		if (c_menu::get().settings == nullptr) {
			D3DXCreateTextureFromFileInMemoryEx(
				pDevice,
				...
				D3DX_DEFAULT, D3DX_DEFAULT,
				NULL, (D3DXIMAGE_INFO*)NULL,
				(PALETTEENTRY*)NULL, &c_menu::get().settings);
		}
	...
	DWORD original_clmove = (DWORD)DetourFunction((byte*)util::FindSignature(m_xor_str("engine.dll"), m_xor_str("55 8B EC 81 EC ? ? ? ? 53 56 8A F9")), (byte*)hooks::Hooked_CLMove);
