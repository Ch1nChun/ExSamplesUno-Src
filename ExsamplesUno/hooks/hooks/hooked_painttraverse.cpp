...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	if (set_console)
	{
		...
		m_cvar()->FindVar(m_xor_str("developer"))->SetValue(FALSE); //-V807
		m_cvar()->FindVar(m_xor_str("con_filter_enable"))->SetValue(TRUE);
		m_cvar()->FindVar(m_xor_str("con_filter_text"))->SetValue(m_xor_str(""));
		m_engine()->ExecuteClientCmd(m_xor_str("clear"));
	}
	...
	if (log_value != g_cfg.misc.show_default_log)
	{
		...
		if (log_value)
			...
			m_cvar()->FindVar(m_xor_str("con_filter_text"))->SetValue(m_xor_str("IrWL5106TZZKNFPz4P4Gl3pSN?J370f5hi373ZjPg%VOVh6lN"));
	}
	...
	if (!HudZoomPanel)
		if (!strcmp(m_xor_str("HudZoom"), m_panel()->GetName(panel)))
			HudZoomPanel = panel;
	...
	if (!panel_id)
	{
		...
		if (!strcmp(panelName, m_xor_str("MatSystemTopPanel")))
			panel_id = panel;
	}
	...
	if (reload_fonts())
	{
		...
		fonts[LOGS] = create_font(m_xor_str("Lucida Console"), 10, FW_MEDIUM, FONTFLAG_DROPSHADOW);
		fonts[ESP] = create_font(m_xor_str("Small Fonts"), 8, FW_MEDIUM, FONTFLAG_OUTLINE);
		...
		fonts[DAMAGE_MARKER] = create_font(m_xor_str("Verdana"), 16, FW_HEAVY, FONTFLAG_OUTLINE);
	...
	if (panel_id == panel)
	{
		...
		if (nci)
		{
			...
			if (latency) //-V550
			{
				static auto cl_updaterate = m_cvar()->FindVar(m_xor_str("cl_updaterate"));
		...
		if (g_ctx.globals.loaded_script)
			for (auto current : c_lua::get().hooks.getHooks(m_xor_str("on_paint")))
				current.func();
