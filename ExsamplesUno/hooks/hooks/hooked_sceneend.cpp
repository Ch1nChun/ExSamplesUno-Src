...
void __fastcall hooks::hooked_sceneend(void* ecx, void* edx)
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		{
			m_materialsystem()->FindMaterial(m_xor_str("Nothing_chams"), nullptr), //-V807
			m_materialsystem()->FindMaterial(m_xor_str("Nothing_metallic"), nullptr),
			m_materialsystem()->FindMaterial(m_xor_str("Nothing_flat"), nullptr),
			m_materialsystem()->FindMaterial(m_xor_str("models/inventory_items/dogtags/dogtags_outline"), nullptr),
			...
			m_materialsystem()->FindMaterial(m_xor_str("Nothing_glow"), nullptr)
		};
