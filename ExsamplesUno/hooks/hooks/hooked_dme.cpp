...
IMaterial* CreateMaterial(bool lit, const std::string& material_data)
{
	...
	std::string type = lit ? m_xor_str("VertexLitGeneric") : m_xor_str("UnlitGeneric");

	auto matname = m_xor_str("Nothing_") + std::to_string(created);
	++created;

	auto keyValues = new KeyValues(matname.c_str());
	static auto key_values_address = util::FindSignature(m_xor_str("client.dll"), m_xor_str("55 8B EC 51 33 C0 C7 45"));

	using KeyValuesFn = void(__thiscall*)(void*, const char*);
	reinterpret_cast <KeyValuesFn> (key_values_address)(keyValues, type.c_str());

	static auto load_from_buffer_address = util::FindSignature(m_xor_str("client.dll"), m_xor_str("55 8B EC 83 E4 F8 83 EC 34 53 8B 5D 0C 89"));
...
void __stdcall hooks::hooked_dme(IMatRenderContext* ctx, const DrawModelState_t& state, const ModelRenderInfo_t& info, matrix3x4_t* bone_to_world)
{
	...
	{
		CreateMaterial(true, m_xor_str(R"#("VertexLitGeneric"
			{
		...
		CreateMaterial(false, m_xor_str(R"#("UnlitGeneric"
			{
				"$basetexture"				"vgui/white"
				"$ignorez"					"0"
				"$envmap"					" "
		...
		m_materialsystem()->FindMaterial(m_xor_str("models/inventory_items/dogtags/dogtags_outline"), nullptr), //-V807
		m_materialsystem()->FindMaterial(m_xor_str("models/inventory_items/trophy_majors/crystal_clear"), nullptr),
		m_materialsystem()->FindMaterial(m_xor_str("models/inventory_items/cologne_prediction/cologne_prediction_glass"), nullptr),
		m_materialsystem()->FindMaterial(m_xor_str("dev/glow_armsrace.vmt"), nullptr),
		m_materialsystem()->FindMaterial(m_xor_str("models/inventory_items/wildfire_gold/wildfire_gold_detail"), nullptr),
		CreateMaterial(true, m_xor_str(R"#("VertexLitGeneric" 
			{ 
				"$additive"					"1" 
