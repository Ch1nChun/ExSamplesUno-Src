...
bool __fastcall hooks::hooked_ishltv(void* ecx, void* edx) 
{
	...
	static auto accumulate_layers_call = util::FindSignature(m_xor_str("client.dll"), m_xor_str("84 C0 75 0D F6 87"));
	static auto setup_velocity_call = util::FindSignature(m_xor_str("client.dll"), m_xor_str("84 C0 75 38 8B 0D ? ? ? ? 8B 01 8B 80 ? ? ? ? FF D0"));
