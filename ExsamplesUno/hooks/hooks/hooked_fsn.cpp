...
void weather()
{
	...
	static auto m_nPrecipType = netg_cfg::get().get_offset(m_xor_str("CPrecipitation"), m_xor_str("m_nPrecipType"));
	static auto m_vecMins = netg_cfg::get().get_offset(m_xor_str("CBaseEntity"), m_xor_str("m_vecMins"));
	static auto m_vecMaxs = netg_cfg::get().get_offset(m_xor_str("CBaseEntity"), m_xor_str("m_vecMaxs"));
...
void remove_smoke()
{
	...
	{
		static auto smoke_count = *reinterpret_cast<uint32_t**>(util::FindSignature(m_xor_str("client.dll"), m_xor_str("A3 ? ? ? ? 57 8B CB")) + 0x1);
		*(int*)smoke_count = 0;
	}
...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	{
		static auto r_drawspecificstaticprop = m_cvar()->FindVar(m_xor_str("r_drawspecificstaticprop")); //-V807
		...
		static auto cl_foot_contact_shadows = m_cvar()->FindVar(m_xor_str("cl_foot_contact_shadows")); //-V807

		if (cl_foot_contact_shadows->GetBool())
			cl_foot_contact_shadows->SetValue(FALSE);

		static auto zoom_sensitivity_ratio_mouse = m_cvar()->FindVar(m_xor_str("zoom_sensitivity_ratio_mouse"));
		...
		static auto r_modelAmbientMin = m_cvar()->FindVar(m_xor_str("r_modelAmbientMin"));
		...
		if (current_shot != g_ctx.shots.end())
		{
			if (!current_shot->latency)
			{
				...
				if (!current_shot->hurt_player)
				{
					...
					{
						current_shot->shot_info.result = m_xor_str("Resolver");
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
							eventlogs::get().add(m_xor_str("Missed shot due to resolver"));
#endif
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						current_shot->shot_info.result = m_xor_str("Spread");

						if (current_shot->occlusion)
							eventlogs::get().add(m_xor_str("Missed shot due to spread"));
						else if (current_shot->shot_info.hitchance == 100)
							eventlogs::get().add(m_xor_str("Missed shot due to prediction error"));
						else
							eventlogs::get().add(m_xor_str("Missed shot due to spread"));
					}
				}
			...
			if (g_ctx.globals.loaded_script && current_shot->shot_info.should_log)
			{
				...
				for (auto current : c_lua::get().hooks.getHooks(m_xor_str("on_shot")))
	...
	{
		if (!death_notice)
			death_notice = util::FindHudElement <DWORD>(m_xor_str("CCSGO_HudDeathNotice"));
		...
		if (death_notice)
		{
			...
			if (g_ctx.globals.should_clear_death_notices)
			{
				...
				static auto clear_notices = (Fn)util::FindSignature(m_xor_str("client.dll"), m_xor_str("55 8B EC 83 EC 0C 53 56 8B 71 58"));
