...
{
	return weapon.find(m_xor_str("decoy")) == std::string::npos && weapon.find(m_xor_str("flashbang")) == std::string::npos &&
		weapon.find(m_xor_str("hegrenade")) == std::string::npos && weapon.find(m_xor_str("inferno")) == std::string::npos &&
		weapon.find(m_xor_str("molotov")) == std::string::npos && weapon.find(m_xor_str("smokegrenade")) == std::string::npos;
}
...
void C_HookedEvents::FireGameEvent(IGameEvent* event)
{
	...
	if (!strcmp(event->GetName(), m_xor_str("player_footstep")))
	{
		auto userid = m_engine()->GetPlayerForUserID(event->GetInt(m_xor_str("userid")));
		...
		if (e->valid(false, false))
		{
			...
			{
				static auto model_index = m_modelinfo()->GetModelIndex(m_xor_str("sprites/physbeam.vmt"));

				if (g_ctx.globals.should_update_beam_index)
					model_index = m_modelinfo()->GetModelIndex(m_xor_str("sprites/physbeam.vmt"));

				BeamInfo_t info;

				info.m_nType = TE_BEAMRINGPOINT;
				info.m_pszModelName = m_xor_str("sprites/physbeam.vmt");
	...
	else if (!strcmp(event_name, m_xor_str("weapon_fire")))
	{
		auto user_id = event->GetInt(m_xor_str("userid"));
	...
	else if (!strcmp(event_name, m_xor_str("bullet_impact")))
	{
		auto user_id = event->GetInt(m_xor_str("userid"));
		...
		if (user == m_engine()->GetLocalPlayer())
		{
			Vector position(event->GetFloat(m_xor_str("x")), event->GetFloat(m_xor_str("y")), event->GetFloat(m_xor_str("z")));
	...
	else if (!strcmp(event_name, m_xor_str("player_death")))
	{
		auto attacker = event->GetInt(m_xor_str("attacker"));
		auto user = event->GetInt(m_xor_str("userid"));
		...
		if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			{
				auto headshot = event->GetBool(m_xor_str("headshot"));

				switch (g_ctx.globals.kills)
				{
				case 0:
					m_surface()->PlaySound_(headshot ? m_xor_str("kill2.wav") : m_xor_str("kill1.wav"));
					break;
				case 1:
					m_surface()->PlaySound_(m_xor_str("kill3.wav"));
					break;
				case 2:
					m_surface()->PlaySound_(m_xor_str("kill4.wav"));
					break;					
				case 3:						
					m_surface()->PlaySound_(m_xor_str("kill5.wav"));
					break;					
				case 4:						
					m_surface()->PlaySound_(m_xor_str("kill6.wav"));
					break;				
				case 5:					
					m_surface()->PlaySound_(m_xor_str("kill7.wav"));
					break;				
				case 6:					
					m_surface()->PlaySound_(m_xor_str("kill8.wav"));
					break;				
				case 7:					
					m_surface()->PlaySound_(m_xor_str("kill9.wav"));
					break;					
				case 8:						
					m_surface()->PlaySound_(m_xor_str("kill10.wav"));
					break;				
				default:				
					m_surface()->PlaySound_(m_xor_str("kill11.wav"));
					break;
				}
			}
			...
			std::string weapon_name = event->GetString(m_xor_str("weapon"));
			auto weapon = g_ctx.local()->m_hActiveWeapon().Get();

			if (weapon && weapon_name.find(m_xor_str("knife")) != std::string::npos)
				SkinChanger::overrideHudIcon(event);
		}
	}
	else if (!strcmp(event_name, m_xor_str("player_hurt")))
	{
		auto attacker = event->GetInt(m_xor_str("attacker"));
		auto user = event->GetInt(m_xor_str("userid"));
		...
		if (attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			if (g_cfg.esp.hitsound && g_cfg.player.enable)
			{
				...
				{
				case 1:
					m_surface()->PlaySound_(m_xor_str("metallic.wav"));
					break;
				case 2:
					m_surface()->PlaySound_(m_xor_str("cod.wav"));
					break;
				case 3:
					m_surface()->PlaySound_(m_xor_str("bubble.wav"));
					break;
				case 4:
					m_surface()->PlaySound_(m_xor_str("stapler.wav"));
					break;
				case 5:
					m_surface()->PlaySound_(m_xor_str("bell.wav"));
					break;
				case 6:
					m_surface()->PlaySound_(m_xor_str("flick.wav"));
					break;
				}
			}
			...
			auto damage = event->GetInt(m_xor_str("dmg_health"));
			auto hitgroup = event->GetInt(m_xor_str("hitgroup"));
			...
			static auto get_hitbox_name = [](int hitbox) -> std::string
			{
				...
				case HITBOX_RIGHT_FOOT:
					return m_xor_str("Left foot");
				case HITBOX_LEFT_FOOT:
					return m_xor_str("Right foot");
				}
			};
			...
			if (current_shot)
			{
				current_shot->shot_info.result = m_xor_str("Hit");
	...
	else if (!strcmp(event_name, m_xor_str("round_start")))
	{
	...
	else if (!strcmp(event_name, m_xor_str("round_freeze_end")))
		antiaim::get().freeze_check = false;
	else if (!strcmp(event_name, m_xor_str("bomb_defused")))
...
void C_HookedEvents::RegisterSelf()
{
	...
	eventmanager->AddListener(this, m_xor_str("player_footstep"), false);
	eventmanager->AddListener(this, m_xor_str("player_hurt"), false);
	eventmanager->AddListener(this, m_xor_str("player_death"), false);
	eventmanager->AddListener(this, m_xor_str("weapon_fire"), false);
	eventmanager->AddListener(this, m_xor_str("item_purchase"), false);
	eventmanager->AddListener(this, m_xor_str("bullet_impact"), false);
	eventmanager->AddListener(this, m_xor_str("round_start"), false);
	eventmanager->AddListener(this, m_xor_str("round_freeze_end"), false);
	eventmanager->AddListener(this, m_xor_str("bomb_defused"), false);
	eventmanager->AddListener(this, m_xor_str("bomb_begindefuse"), false);
	eventmanager->AddListener(this, m_xor_str("bomb_beginplant"), false);

	g_ctx.globals.events.emplace_back(m_xor_str("player_footstep"));
	g_ctx.globals.events.emplace_back(m_xor_str("player_hurt"));
	g_ctx.globals.events.emplace_back(m_xor_str("player_death"));
	g_ctx.globals.events.emplace_back(m_xor_str("weapon_fire"));
	g_ctx.globals.events.emplace_back(m_xor_str("item_purchase"));
	g_ctx.globals.events.emplace_back(m_xor_str("bullet_impact"));
	g_ctx.globals.events.emplace_back(m_xor_str("round_start"));
	g_ctx.globals.events.emplace_back(m_xor_str("round_freeze_end"));
	g_ctx.globals.events.emplace_back(m_xor_str("bomb_defused"));
	g_ctx.globals.events.emplace_back(m_xor_str("bomb_begindefuse"));
	g_ctx.globals.events.emplace_back(m_xor_str("bomb_beginplant"));
}
