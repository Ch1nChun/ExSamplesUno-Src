...
_declspec(noinline)bool hooks::setupbones_detour(void* ecx, matrix3x4_t* bone_world_out, int max_bones, int bone_mask, float current_time)
{
	...
	static auto r_jiggle_bones = m_cvar()->FindVar(m_xor_str("r_jiggle_bones"));
...
{
	static auto allow_extrapolation = *(bool**)(util::FindSignature(m_xor_str("client.dll"), m_xor_str("A2 ? ? ? ? 8B 45 E8")) + 0x1);
