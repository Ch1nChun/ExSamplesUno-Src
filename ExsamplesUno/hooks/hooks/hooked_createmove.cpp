...
bool __stdcall hooks::hooked_createmove(float smt, CUserCmd* m_pcmd)
{
	...
	g_ctx.globals.exploits = misc::get().double_tap_key;
	...
	if (!g_ctx.globals.weapon->is_non_aim())
	{
		...
		{
			static auto weapon_recoil_scale = m_cvar()->FindVar(m_xor_str("weapon_recoil_scale"));
	...
	if (g_ctx.globals.loaded_script)
		for (auto current : c_lua::get().hooks.getHooks(m_xor_str("on_createmove")))
			current.func();
	...
	if (g_cfg.misc.buybot_enable && g_ctx.globals.should_buy)
	{
		...
		if (!g_ctx.globals.should_buy)
		{
			...
			{
			case 1:
				buy += m_xor_str("buy g3sg1; ");
				break;
			case 2:
				buy += m_xor_str("buy awp; ");
				break;
			case 3:
				buy += m_xor_str("buy ssg08; ");
				break;
			}
			...
			{
			case 1:
				buy += m_xor_str("buy elite; ");
				break;
			case 2:
				buy += m_xor_str("buy deagle; buy revolver; ");
				break;
			}

			if (g_cfg.misc.buybot3[BUY_ARMOR])
				buy += m_xor_str("buy vesthelm; buy vest; ");

			if (g_cfg.misc.buybot3[BUY_TASER])
				buy += m_xor_str("buy taser; ");

			if (g_cfg.misc.buybot3[BUY_GRENADES])
				buy += m_xor_str("buy molotov; buy hegrenade; buy smokegrenade; buy flashbang; buy flashbang; buy decoy; ");

			if (g_cfg.misc.buybot3[BUY_DEFUSER])
				buy += m_xor_str("buy defuser; ");
