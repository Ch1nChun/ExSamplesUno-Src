...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, bool minimum_damage, float power)
{
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        window->DrawList->AddRectFilled(ImVec2(frame_bb.Min.x, frame_bb.Min.y + 9), ImVec2(grab_bb.Max.x - 2, grab_bb.Max.y - 9), ImColor(130, 125, 253), (15, 15, 15, 253), (15, 15, 15, 253));
        window->DrawList->AddRectFilled(ImVec2(grab_bb.Min.x - 2, grab_bb.Min.y + 6), ImVec2(grab_bb.Max.x, grab_bb.Max.y - 8), ImColor(25, 25, 25, 253), (25, 25, 25, 253), (25, 25, 25, 253));
    }
    // Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);
    ...
    {
        auto value_damage = m_xor_str("HP + ") + std::to_string(*(int*)p_data - 100);
...
bool ImGui::PlusButton(const char* label, bool type, const ImVec2& size_arg, const char* uniq_id, bool config)
{
    ImColor col = ImColor(74 / 255.f, 165 / 255.f, 218 / 255.f);
    ...
    if (!config)
        PushFont(c_menu::get().smallest_pixel);
    if (!config)
...
bool ImGui::TabEx(const char* icon, const char* label, const bool active, const ImVec2& size_arg)
{
    bool selected;
    ...
    it_filled->second = clamp(it_filled->second + (2.55f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.0f)), it_hover->second, 1.f);
    ...
    it_fill->second = clamp(it_filled->second + (1.75f * ImGui::GetIO().DeltaTime * (selected ? 1.f : -1.0f)), it_hover->second, 1.f);
    ...
    ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(bb.Min.x - 2 * it_filled->second + 3, bb.Min.y), ImVec2(bb.Min.x + 12, bb.Max.y), ImColor(130, 125, 253, int(95 * it_fill->second)), ImColor(130, 125, 253, int(5 * it_fill->second)), ImColor(130, 125, 253, int(5 * it_fill->second)), ImColor(130, 125, 253, int(95 * it_fill->second)));
    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y), ImVec2(bb.Min.x + 1 + it_filled->second * 3, bb.Max.y), ImColor(130, 125, 253, int(255 * it_filled->second)), 6, ImDrawFlags_RoundCornersRight);
    ...
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + 50, bb.Min.y + (15 - CalcTextSize(label).y / 2)), ImColor(text2), label);
    ImGui::PopFont();
...
{
    return TabEx(icon, label, active, size_arg);
}
...
bool ImGui::MenuChilds(const char* str_id, const ImVec2& size_arg, bool sub_tab, ImGuiWindowFlags extra_flags)
{
    ImGuiWindow* window = GetCurrentWindow();
    return beginchildexs(str_id, window->GetID(str_id), size_arg, sub_tab, extra_flags);
}
...
bool ImGui::beginchildex(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    parent_window->DrawList->AddRectFilled(pos + ImVec2(0, 5), pos + size_arg, ImColor(14, 14, 14, int(255 * alpha)), 1);
    parent_window->DrawList->AddRectFilledMultiColor(pos + ImVec2(0, 21), pos + ImVec2(size_arg.x, 22), ImColor(130, 125, 253, 100), ImColor(130, 125, 253, 100), ImColor(130, 125, 253, 100), ImColor(130, 125, 253, 100));
    parent_window->DrawList->AddText(pos + ImVec2(6, 7), ImColor(255, 255, 255, int(255 * alpha)), name);
...
bool ImGui::beginchildexs(const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags)
{
    ...
    auto label_size = CalcTextSize(name);
    auto pos = ImGui::GetWindowPos() - ImVec2(0, 14);
    parent_window->DrawList->AddRectFilled(pos, pos + size_arg, ImColor(14, 14, 14, int(255 * alpha)), 1);
...
bool ImGui::subtabs(const char* label, bool selected, float sizex, int cout, bool reverse)
{
    ...
    //37, 37, 40
    ImVec4 col = ImLerp(ImVec4{ 94 / 255.f, 94 / 255.f, 94 / 255.f, 0.7f * al }, ImVec4{ 44 / 255.f, 44 / 255.f, 47 / 255.f, 0.5f * al }, t * al);
    ...
    if (it_filled->second > 0)
        if (!reverse)
            ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x + size.x * min(t * 2, 1), bb.Min.y + 2 * min(t, 1)), ImColor((135, 125, 253), (135, 125, 253), (135, 125, 253), (int)((255 * t) * al)), 12, ImDrawCornerFlags_Bot);
...
bool ImGui::subtab(const char* label, bool selected)
{
    ...
    it_filled->second = math::clamp(it_filled->second + (2.15f * GetIO().DeltaTime * (selected ? 1.f : -1.2f)), 0.0f, 1.f);
    it_filled->second *= al;
    static std::map<ImGuiID, float> hover_animation;
    auto it_hover = hover_animation.find(id);
    if (it_hover == hover_animation.end())
    ...
    it_hover->second = math::clamp(it_hover->second + (2.15f * GetIO().DeltaTime * (hovered ? 1.f : -1.f)), 0.0f, 1.f);
    it_hover->second *= al;
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(3, 3), bb.Max + ImVec2(3, 3), ImColor(130, 125, 253, int((5 * al) * it_filled->second)), 6);
    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(2, 2), bb.Max + ImVec2(2, 2), ImColor(130, 125, 253, int((15 * al) * it_filled->second)), 6);
    GetWindowDrawList()->AddRectFilled(bb.Min - ImVec2(1, 1), bb.Max + ImVec2(1, 1), ImColor(130, 125, 253, int((25 * al) * it_filled->second)), 6);
    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, ImColor(130, 125, 253, int((205 * al) * it_filled->second)), 6);
    ...
    if (selected)
        GetWindowDrawList()->AddText(ImVec2(bb.Min.x + (size.x / 2 - label_size.x / 2), bb.Min.y + (25 / 2 - label_size.y / 2)), ImColor(220, 220, 220, int((255 * al) * it_filled->second)), label);
    return pressed;
}
...
bool ImGui::tab(const char* icon, const char* label, bool selected)
{
    ...
    ImVec4 text = ImLerp(ImVec4{ 158 / 255.f, 157 / 255.f, 161 / 255.f, GetStyle().Alpha }, ImVec4{ 130 / 255.f, 125 / 255.f, 253 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImVec4 text2 = ImLerp(ImVec4{ 158 / 255.f, 157 / 255.f, 161 / 255.f, GetStyle().Alpha }, ImVec4{ 220 / 255.f, 220 / 255.f, 220 / 255.f, GetStyle().Alpha }, it_filled->second);
    ImGui::GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(bb.Min.x - 2 * it_filled->second + 3, bb.Min.y), ImVec2(bb.Min.x + 12, bb.Max.y), ImColor(130, 125, 253, int(95 * it_fill->second)), ImColor(130, 125, 253, int(5 * it_fill->second)), ImColor(130, 125, 253, int(5 * it_fill->second)), ImColor(130, 125, 253, int(95 * it_fill->second)));
    ImGui::GetForegroundDrawList()->AddRectFilled(ImVec2(bb.Min.x + 1, bb.Min.y), ImVec2(bb.Min.x + 1 + it_filled->second * 3, bb.Max.y), ImColor(130, 125, 253, int(255 * it_filled->second)), 6, ImDrawFlags_RoundCornersRight);
    ImGui::GetForegroundDrawList()->AddText(ImVec2(bb.Min.x + (25 - label_size.x / 2), bb.Min.y + (15 - label_size.y / 2)), ImColor(text), icon);
