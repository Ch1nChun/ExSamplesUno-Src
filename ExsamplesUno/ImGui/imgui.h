...
namespace ImGui
{
    ...
    IMGUI_API bool          PlusButton(const char* label, bool type = false, const ImVec2& size_arg = ImVec2(0, 0), const char* uniq_id = "", bool config = false);
    ...
    IMGUI_API bool          MenuChilds(const char* str_id, const ImVec2& size_arg = ImVec2(0, 0), bool sub_tab = false, ImGuiWindowFlags extra_flags = NULL);
    IMGUI_API bool          subtab(const char* label, bool selected);
    IMGUI_API bool          subtabs(const char* label, bool selected, float sizex, int cout = 2, bool reverse = false);

    bool MultiCombo(const char* label, int current_item, const char* items[], int items_count, bool combos[]);
    IMGUI_API bool          TabEx(const char* icon, const char* label, const bool active = false, const ImVec2& size_arg = ImVec2(0, 0));
    // Tab Bars, Tabs
