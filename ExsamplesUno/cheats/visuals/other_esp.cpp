...
void otheresp::grenade_tracer()
{
	...
	misc::get().force_sv_cheats();

	auto grenade_trajectory = m_cvar()->FindVar("sv_grenade_trajectory");
	auto grenade_trajectory_time = m_cvar()->FindVar("sv_grenade_trajectory_time");

	if (g_cfg.esp.grenade_trail == true)
	{
		grenade_trajectory->SetValue(1);
		grenade_trajectory_time->SetValue(6);
	}
	else
	{
		grenade_trajectory->SetValue(0);
		grenade_trajectory_time->SetValue(20);
	}
}
