...
{
	static auto load_skybox = reinterpret_cast<void(__fastcall*)(const char*)>(util::FindSignature(m_xor_str("engine.dll"), m_xor_str("55 8B EC 81 EC ? ? ? ? 56 57 8B F9 C7 45")));
	...
	if (changed)
	{
		...
		for (auto i = materialsystem->FirstMaterial(); i != materialsystem->InvalidMaterial(); i = materialsystem->NextMaterial(i))
		{
			...
			if (strstr(material->GetTextureGroupName(), m_xor_str("SkyBox")))
...
{
	static auto fog_override = m_cvar()->FindVar(m_xor_str("fog_override")); //-V807
	...
	static auto fog_start = m_cvar()->FindVar(m_xor_str("fog_start"));

	if (fog_start->GetInt())
		fog_start->SetValue(0);

	static auto fog_end = m_cvar()->FindVar(m_xor_str("fog_end"));

	if (fog_end->GetInt() != g_cfg.esp.fog_distance)
		fog_end->SetValue(g_cfg.esp.fog_distance);

	static auto fog_maxdensity = m_cvar()->FindVar(m_xor_str("fog_maxdensity"));
	...
	static auto fog_color = m_cvar()->FindVar(m_xor_str("fog_color"));
...
void worldesp::bomb_timer(entity_t* entity)
{
	...
	static auto mp_c4timer = m_cvar()->FindVar(m_xor_str("mp_c4timer"));
