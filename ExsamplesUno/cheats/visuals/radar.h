...
public:
	bool OnMapLoad(const char* pszMapName, IDirect3DDevice9* pDevice)
	{
		...
		sprintf(szPath, m_xor_str("csgo\\resource\\overviews\\%s_radar.dds"), pszMapName);
		...
		sprintf(szPath, m_xor_str(".\\csgo\\resource\\overviews\\%s.txt"), pszMapName);
		...
		m_vMapOrigin.x = std::stoi(parseString(m_xor_str("\"pos_x\""), szInfo));
		m_vMapOrigin.y = std::stoi(parseString(m_xor_str("\"pos_y\""), szInfo));
		m_flMapScale = std::stod(parseString(m_xor_str("\"scale\""), szInfo));

		auto bombA_x = parseString(m_xor_str("\"bombA_x\""), szInfo);

		if (!bombA_x.empty())
		{
			bomb_siteA.x = std::stod(parseString(m_xor_str("\"bombA_x\""), szInfo));
			bomb_siteA.y = std::stod(parseString(m_xor_str("\"bombA_y\""), szInfo));
			bomb_siteA.z = 0.0f;

			draw_plantA = true;
		}

		auto bombB_x = parseString(m_xor_str("\"bombB_x\""), szInfo);

		if (!bombB_x.empty())
		{
			bomb_siteB.x = std::stod(parseString(m_xor_str("\"bombB_x\""), szInfo));
			bomb_siteB.y = std::stod(parseString(m_xor_str("\"bombB_y\""), szInfo));
			bomb_siteB.z = 0.0f;
