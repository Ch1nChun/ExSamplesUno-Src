...
void engineprediction::setup()
{
	...
	if (!prediction_data.sv_footsteps)
		prediction_data.sv_footsteps = m_cvar()->FindVar(m_xor_str("sv_footsteps"));

	if (!prediction_data.sv_min_jump_landing_sound)
		prediction_data.sv_min_jump_landing_sound = m_cvar()->FindVar(m_xor_str("sv_min_jump_landing_sound"));
...
void engineprediction::predict(CUserCmd* m_pcmd)
{
	...
	int m_nimpulse = util::find_in_datamap(g_ctx.local()->GetPredDescMap(), m_xor_str("m_nImpulse"));
	int get_buttons = util::find_in_datamap(g_ctx.local()->GetPredDescMap(), m_xor_str("m_nButtons"));
	int get_buttons_last = util::find_in_datamap(g_ctx.local()->GetPredDescMap(), m_xor_str("m_afButtonLast"));
	int get_buttons_pressed = util::find_in_datamap(g_ctx.local()->GetPredDescMap(), m_xor_str("m_afButtonPressed"));
	int get_buttons_released = util::find_in_datamap(g_ctx.local()->GetPredDescMap(), m_xor_str("m_afButtonReleased"));
	...
	*reinterpret_cast<CUserCmd**>(reinterpret_cast<uintptr_t>(g_ctx.local()) + 0x3288) = m_pcmd;
	...
	if (!prediction_data.prediction_random_seed)
		prediction_data.prediction_random_seed = *reinterpret_cast <int**> (util::FindSignature(m_xor_str("client.dll"), m_xor_str("A3 ? ? ? ? 66 0F 6E 86")) + 0x1);
	...
	*prediction_data.prediction_random_seed = MD5_PseudoRandom(m_pcmd->m_command_number) & INT_MAX;
	...
	if (!prediction_data.prediction_player)
		prediction_data.prediction_player = *reinterpret_cast <int**> (util::FindSignature(m_xor_str("client.dll"), m_xor_str("89 35 ? ? ? ? F3 0F 10 48")) + 0x2);
